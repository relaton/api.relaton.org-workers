...
- [ ] Measure working-tree sizes of relaton-data-{rfcs,rfcsubseries,ids}
...
- [ ] Branch `unified` in relaton-data-ietf (fresh clone).
- [ ] Populate streams: `data/{rfcs,rfcsubseries,ids}/` copied verbatim from
      the type repos (v1.5.6 YAML, no conversion).
- [ ] Bootstrap `data/misc/`: one-time upgrade of
...
- [ ] `tools/build_index.rb` in-repo: emits `index.yaml` + `index.zip` with
...
- [ ] `.github/workflows/aggregate.yml`: daily cron, lags source crawls —
      fast-forward stream subdirs, rebuild index, commit. Replaces legacy
      bibxml `crawler.yml`; drop `keep-alive.yml`.
- [ ] Tag `bibxml-final` on master; PR `unified` → master (deletes
